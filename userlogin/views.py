...
def Register(request):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('userlogin:login')) #sending the msg to differnt page and displaying message
    form = forms.Reg()
...
def Login(request):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            print(usermail)
            print(password)
            p1=models.login.objects.filter(email= usermail, password=password,)
            u,p = 0,0
            for i in p1:
                u=i.email
                p=i.password
            if usermail==u and password==p:
                ...
                return HttpResponseRedirect('userlogin:login?error=1')
